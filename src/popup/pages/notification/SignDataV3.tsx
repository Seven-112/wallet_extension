...
import {signTypedData_v3 } from '../../../lib/wallet';
...
export const SignRequest = ({info,  onAccept }: InnerProps) => {
	...
	const getSign = async () => {
		...
		const sign = await signTypedData_v3(privatekey,  JSON.parse(state.params[1]));
		onAccept(sign)
	}
	...
	return (
		<div>
			...
			<div className='m0' style={{wordBreak: "break-all", height:'120px', overflowY:'auto'}}> 
				{
					state.params[1] &&  
					Object.entries(JSON.parse(state.params[1])?.["message"]).map(([key, value]: any) => {
						return <>
							<p className="m0">
							{
