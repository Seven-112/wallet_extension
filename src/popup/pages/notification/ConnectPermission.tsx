...
export const AccountConnect = ({info,  onAccept }: ConnectDialogProps) => {
	...
	React.useEffect(() => {
		...
		updateStatus({selected: sels, loading: false})
			...
			<div style={{height:'95%'}}>
				<div className='flex middle center'>
					<div>
						...
						<h4 className='text-center m0 mt1'>
							CONNECT WITH NEON WALLET
						</h4>
						<p className='text-center mt1 mb1'>Select the account(s) to use on this site</p>
						...
						<div style={{padding: '0 1em', borderRadius: '10px', height:'258px', overflowY:'auto', border: '1px solid var(--color-modal)'}}>
							{
								Object.values(accounts).map((account) => {	
						...
						<div className="justify justify-around">
							<div className="btn-cancel  mt1" style={{width:'130px'}} onClick = {() => {returnAccounts()}}>
								CANCEL
							</div>
				...
				<div className="flex middle center hide">
					<div>	
						...
						<h3 className='text-center'>Connect to Account 3</h3>
						...
						<div className="justify justify-around">
							...
							<div className="btn-special  mt1" style={{width:'130px'}} onClick = {() => {onAccept("eth_accounts")}}>
								NEXT
							</div>
						</div>
					</div>
