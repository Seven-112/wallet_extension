...
export const SignRequest = ({info,  onAccept }: InnerProps) => {
	...
	const getSign = async () => {
		...
		const sign = await signTypedData(privatekey, (state.params[0]));
		onAccept(sign)
	}
	...
	return (
		<div>
			...
			<div className='m0' style={{wordBreak: "break-all", height:'120px', overflowY:'auto'}}> 
				{
					state.params[0] && Object.values(state.params[0]).map((param: any) => { 
						return <> <p className="m0">{param?.["name"] + ": " + param?.["value"]}</p> <br></br></>
					})
				}
			</div>
